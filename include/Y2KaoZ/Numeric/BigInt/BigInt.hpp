...
[[nodiscard]] Y2KAOZNUMERIC_EXPORT auto fromBuffer(std::span<const boost::endian::little_uint8_buf_t> buffer) -> BigInt;
[[nodiscard]] Y2KAOZNUMERIC_EXPORT auto fromBuffer(std::span<const boost::endian::big_uint8_buf_t> buffer) -> BigInt;
...
[[nodiscard]] Y2KAOZNUMERIC_EXPORT auto byteBuffer(const BigInt& bigInt, bool bigEndian = false)
  -> std::vector<std::byte>;
[[nodiscard]] Y2KAOZNUMERIC_EXPORT auto littleEndianBuffer(const BigInt& bigInt)
  -> std::vector<boost::endian::little_uint8_buf_t>;
[[nodiscard]] Y2KAOZNUMERIC_EXPORT auto bigEndianBuffer(const BigInt& bigInt)
  -> std::vector<boost::endian::big_uint8_buf_t>;

Y2KAOZNUMERIC_EXPORT template <std::size_t SIZE>
...
Y2KAOZNUMERIC_EXPORT template <std::size_t SIZE>
inline auto littleEndianBuffer(const BigInt& bigInt) -> std::array<boost::endian::little_uint8_buf_t, SIZE> {
  return std::bit_cast<std::array<boost::endian::little_uint8_buf_t, SIZE>>(uintBuffer<SIZE>(bigInt, false));
}

Y2KAOZNUMERIC_EXPORT template <std::size_t SIZE>
inline auto bigEndianBuffer(const BigInt& bigInt) -> std::array<boost::endian::big_uint8_buf_t, SIZE> {
...
Y2KAOZNUMERIC_EXPORT template <typename T>
inline auto powm(const T& a, const T& p, const T& c) -> T {
